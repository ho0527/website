    ...
    <body>
        ...
        <table class="formtable">
            ...
            for($i=0;$i<count($row);$i=$i+1){
                if(query($db,"SELECT*FROM `questioncode` WHERE `user`!='$data'AND`user`!=''")){}
                else{
                    $id=$row[$i][0];
                    ...
                    <tr id="<?php echo($id) ?>">
                        ...
                        <td class="formtd">
                            ...
                            <input type="button" class="workbutton" onclick="location.href='?mod=respone&id=<?php echo($row[$i][0]) ?>'" value="填寫問卷"><br>
                        </td>
                    </tr>
                    <?php
                }
            ...
            if(isset($_GET["mod"])){
                ...
                if($mod=="lock"||$mod=="edit"||$mod=="del"){
                    if($row[5]=="false"){
                        ...
                        }else{ ?><script>alert("[ERROR]PERMISSION ERROR(WTF你怎麼用的)(ERROR CODE:403)");location.href="admin.php"</script><?php }
                    }elseif($row[5]=="true"){
                        if($mod=="lock"){
                            query($db,"UPDATE `question` SET `lock`='false' WHERE `id`='$id'");
                        }elseif($mod=="edit"||$mod=="del"){ ?><script>alert("[ERROR]PERMISSION ERROR(幹不要亂搞)(ERROR CODE:403)");location.href="admin.php"</script><?php
                        }else{ ?><script>alert("[ERROR]PERMISSION ERROR(WTF你怎麼用的)(ERROR CODE:403)");location.href="admin.php"</script><?php }
                    ...
                    query($db,"INSERT INTO `question`(`title`,`questioncount`,`pagelen`,`lock`,`maxlen`)VALUES(?,?,?,?,?)",[$title,$row[2],$row[3],"false",$row[6]]);
                    ...
                    ?><script>alert("[ERROR]REQUST NOT FOUND(幹不要亂搞)(ERROR CODE:403)");location.href="admin.php"</script><?php
                }
            }
        ?>
