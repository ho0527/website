...
<body>
    ...
    <div class="timer">
        <form action="">
            <input type="text" class="mtimer" name="" id="timer" value="<?= $_SESSION["timer"] ?>" readonly>s
            <input type="text" name="timer" id="" value="<?= $_SESSION["timer"] ?>">
    ...
    <div class="lightbox">
        <div class="main2">
            是否繼續操作?<br>
            <input type="button" class="but" onclick="location.reload()" value="Yes">
            <input type="button" class="but" onclick="location.href='link.php?logout='" value="否">
        </div>
    </div>
    <div class="main mag">
        <h2>會員管理</h2>
        ...
        <form action="">
            <table>
                <tr>
                    <td class="atd">使用者編號<input type="submit" name="nb" id="nb" value="降冪"></td>
                    <td class="atd">使用者帳號<input type="submit" name="un" id="un" value="降冪"></td>
                    <td class="atd">使用者密碼</td>
                    <td class="atd">使用者姓名<input type="submit" name="n" id="n" value="降冪"></td>
                    <td class="atd">使用者權限</td>
                </tr>
                <?php
                    if(isset($_SESSION["search"])){
                        $type=$_SESSION["search"];
                        updown(fetchall(query($db,"SELECT*FROM `user` WHERE `name`LIKE'%$type%'or`username`LIKE'%$type%'or`code`LIKE'%$type%'or`permission`LIKE'%$type%'or`number`LIKE'%$type%'")));
                    }else{
