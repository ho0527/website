    ...
    if(!block($username)){
        if($row=fetch(query($db,"SELECT*FROM `user` WHERE `username`='$username'"))){
            if($row[3]==$code){
                $verify=str_split($_SESSION["verify"]);
                 ...
                 if($verify==str_split($_GET["verify"])){
                    query($db,"INSERT INTO `data`(`number`,`move`,`movetime`)VALUES('$row[1]','登入成功','$time')");
                    session_unset();
                    $_SESSION["data"]=$row[1];
                    ...
                    if($_SESSION["error"]<3){
                        ...
                        query($db,"INSERT INTO `data`(`number`,`move`,`movetime`)VALUES('$row[1]','登入失敗','$time')");
                        session_unset();
            ...
            if($_SESSION["error"]<3){
                ...
                query($db,"INSERT INTO `data`(`number`,`move`,`movetime`)VALUES('未知','登入失敗','$time')");
                session_unset();
