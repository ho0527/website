    ...
    if($row=fetch(query($db,"SELECT*FROM `user` WHERE `username`='$username'"))){
        if($row[3]==$code){
            ...
            if($verify==str_split($_GET["verify"])){
                query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`, `permission`, `move`, `movertime`) VALUES ('$row[1]','$row[2]','$row[3]','$row[4]','$row[5]','登入成功','$time')");                session_unset();
                $_SESSION["data"]=$row[1];
                ...
                if($_SESSION["error"]<3){
                    ...
                    query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`, `permission`, `move`, `movertime`) VALUES ('$row[1]','$row[2]','$row[3]','$row[4]','$row[5]','登入失敗','$time')");
                    session_unset();
        ...
        if($_SESSION["error"]<3){
            ...
            query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`, `permission`, `move`, `movertime`) VALUES ('未知','','','','','登入失敗','$time')");
            session_unset();
