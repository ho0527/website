...
<body>
    ...
    <hr>
    <form>
        <input type="text" class="mtimer" id="timer" value="<?= $_SESSION["timer"] ?>">s
        <input type="number" name="timer" value="<?= $_SESSION["timer"] ?>">
        <input type="submit" class="but" name="ref" value="送出">
        <input type="button" class="but" onclick="location.reload()" value="重新計時">
    </form><br>
    <div class="lightbox">
        <div class="main2">
            是否繼續操作?<br>
    ...
    <div class="main mag">
        ...
        <form>
            <table>
                <tr>
                    <td class="atd">編號<input type="submit" name="nb" id="nb" value="升冪"></td>
                    <td class="atd">帳號<input type="submit" name="un" id="un" value="升冪"></td>
                    <td class="atd">密碼</td>
                    <td class="atd">姓名<input type="submit" name="n" id="n" value="升冪"></td>
                    <td class="atd">權限</td>
                </tr>
                <?php
                    if(isset($_SESSION["search"])){
                        $type=$_SESSION["search"];
                        updown(fetchall(query($db,"SELECT*FROM `user` WHERE `username`LIKE'%$type%'or`name`LIKE'%$type%'or`number`LIKE'%$type%'or`code`LIKE'%$type%'or`permission`LIKE'%$type%'")));
                    }else{
