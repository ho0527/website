    ...
    if($row=fetch(query($db,"SELECT * FROM `user` WHERE `username`='$username'"))){
        if($row[2]==$code){
            ...
            if($verify==str_split($_GET["verify"])){
                query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`,`premission`, `movetime`, `move`) VALUES ('$row[4]','$row[1]','$row[2]','$row[3]','$row[5]','$time','登入成功')");
                session_unset();
                $_SESSION["data"]=$row[4];
                ...
                if($_SESSION["error"]<3){
                    ...
                    session_unset();
                    query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`,`premission`, `movetime`, `move`) VALUES ('$row[4]','$row[1]','$row[2]','$row[3]','$row[5]','$time','登入失敗')");
        ...
        if($_SESSION["error"]<3){
            ...
            session_unset();
            query($db,"INSERT INTO `data`(`number`, `username`, `code`, `name`, `movetime`, `move`) VALUES ('未知','','','','$time','登入失敗')");
