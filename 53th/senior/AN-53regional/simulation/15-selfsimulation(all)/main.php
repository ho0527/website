...
<body>
    <?php
        ...
        if($_SESSION["permission"]=="管理者"){
            ...
            <div class="main">
                <form action="">
                    數字範圍: <input type="number" name="start" value="0">~<input type="number" name="end" value="100000000000000000">
                ...
                <table class="ptable">
                    <?php
                        if(isset($_GET["num"])){
                            ...
                            $type=$_GET["tex"];
                            $row=fetchall(query($db,"SELECT*FROM `coffee` WHERE `name`LIKE'%$type%'or`cost`LIKE'%$type%'or`time`LIKE'%$type%'or`link`LIKE'%$type%'or`intr`LIKE'%$type%'"));
                        }else{
                        ...
                        for($i=0;$i<count($row);$i++){
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input type="button" class="but" onclick="location.href='edit.php?pedit=<?= $row[$i][0] ?>'" value="修改">
                                </td>
                            </tr>
                            <?php
                        }
